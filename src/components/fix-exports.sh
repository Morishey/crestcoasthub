...
echo "✅ Fixed Business.tsx"

# Fix other components if needed
for file in Header.tsx Hero.tsx Features.tsx About.tsx Products.tsx Lending.tsx WhyChoose.tsx Footer.tsx; do
  if [ -f "$file" ]; then
    # Check if file ends with export default
    if ! tail -5 "$file" | grep -q "export default"; then
      echo "⚠️  $file might need export default check"
    fi
  fi
done
